...
public class CalendarPropsProcessor extends BasePropsProcessor {
	...
	public CalendarPropsProcessor(WebDAVRequest webDAVRequest, Resource resource, Element rootElement) {
	...
	protected void processCalDAVCalendarColor() {
		...
		DocUtil.add(successPropElement, CalDAVProps.CALDAV_CALENDAR_COLOR, color);
	}

	@Override
	protected void processCalDAVCalendarDescription() {
		DocUtil.add(successPropElement, CalDAVProps.CALDAV_CALENDAR_DESCRIPTION, _calendar.getDescription(locale));
	}
	...
	protected void processCalDAVCalendarUserAddressSet() {
		...
		Element calendarHomeSetElement = DocUtil.add(successPropElement, CalDAVProps.CALDAV_CALENDAR_USER_ADDRESS_SET);
	...
	protected void processCalDAVGetCTag() {
		DocUtil.add(successPropElement, CalDAVProps.CALDAV_GETCTAG, CalDAVUtil.getResourceETag(resource));
	}

	@Override
	protected void processCalDAVSupportedCalendarComponentSet() {
		Element supportedCalendarComponentSet = DocUtil.add(successPropElement,
				CalDAVProps.CALDAV_SUPPORTED_CALENDAR_COMPONENT_SET);

		if (!resource.isCollection() || CalDAVUtil.isMacOSX(webDAVRequest)) {
			Element el = DocUtil.add(supportedCalendarComponentSet, CalDAVProps.DAV_COMP);
	...
	protected void processDAVCurrentUserPrivilegeSet() {
		Element currentUserPrivilegeSetElement = DocUtil.add(successPropElement,
				CalDAVProps.DAV_CURRENT_USER_PRIVILEGE_SET);

		Element readPrivilegeElement = DocUtil.add(currentUserPrivilegeSetElement,
				CalDAVProps.createQName("privilege"));

		DocUtil.add(readPrivilegeElement, CalDAVProps.createQName("read"));

		if (CalendarPermission.contains(webDAVRequest.getPermissionChecker(), _calendar, ActionKeys.UPDATE)) {

			DocUtil.add(readPrivilegeElement, CalDAVProps.createQName("write"));

			DocUtil.add(readPrivilegeElement, CalDAVProps.createQName("write-content"));
		}
	}

	@Override
	protected void processDAVGetContentLength() {
		DocUtil.add(successPropElement, CalDAVProps.DAV_GETCONTENTLENGTH, resource.getSize());
	}

	@Override
	protected void processDAVOwner() {
		DocUtil.add(successPropElement, CalDAVProps.DAV_OWNER, CalDAVUtil.getPrincipalURL(_calendar.getUserId()));
	}
	...
	protected void processDAVResourceType() {
		...
		DocUtil.add(resourceTypeElement, CalDAVProps.createCalendarQName("calendar"));
	}

	protected String vTimeZoneToString(net.fortuna.ical4j.model.Calendar iCalCalendar) throws Exception {
	...
	private static Log _log = LogFactoryUtil.getLog(CalendarPropsProcessor.class);
