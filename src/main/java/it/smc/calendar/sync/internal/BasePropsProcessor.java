...
import it.smc.calendar.sync.CalDAVMethod;
...
public abstract class BasePropsProcessor implements PropsProcessor {
	...
	public BasePropsProcessor(WebDAVRequest webDAVRequest, Resource resource, Element rootElement) {

		init(webDAVRequest, resource, rootElement);
	}

	@Override
	public void processProperties(Set<QName> properties) throws PortalException {
		...
		if (props.contains(CalDAVProps.CALDAV_SUPPORTED_CALENDAR_COMPONENT_SET)) {
	...
	protected void processCalDAVCalendarDescription() {
		DocUtil.add(failurePropElement, CalDAVProps.CALDAV_CALENDAR_DESCRIPTION);
	}
	...
	protected void processCalDAVCalendarUserAddressSet() {
		...
		try {
			resource = CalendarResourceLocalServiceUtil.fetchCalendarResource(PortalUtil.getClassNameId(User.class),
		...
		if (resource != null) {
			Element calendarHomeSetElement = DocUtil.add(successPropElement,
			...
			DocUtil.add(failurePropElement, CalDAVProps.CALDAV_CALENDAR_USER_ADDRESS_SET);
		}
	}
	...
	protected void processCalDAVSupportedCalendarComponentSet() {
		DocUtil.add(failurePropElement, CalDAVProps.CALDAV_SUPPORTED_CALENDAR_COMPONENT_SET);
	}

	protected void processCalDAVSupportedCalendarData() {
		DocUtil.add(failurePropElement, CalDAVProps.CALDAV_SUPPORTED_CALENDAR_DATA);
	}
	...
	protected void processDAVCreationDate() {
		DocUtil.add(successPropElement, CalDAVProps.DAV_CREATIONDATE, resource.getCreateDate());
	}

	protected void processDAVCurrentUserPrincipal() {

		if (CalDAVUtil.isIOS(webDAVRequest)) {
			DocUtil.add(failurePropElement, CalDAVProps.DAV_CURRENT_USER_PRINCIPAL);
			return;
		}

		// TODO: review from specifications

		Element principalUrlElement = DocUtil.add(successPropElement, CalDAVProps.DAV_CURRENT_USER_PRINCIPAL);

		DocUtil.add(principalUrlElement, CalDAVProps.createQName("href"),
				CalDAVUtil.getPrincipalURL(webDAVRequest.getUserId()));
	}

	protected void processDAVCurrentUserPrivilegeSet() {
		DocUtil.add(failurePropElement, CalDAVProps.DAV_CURRENT_USER_PRIVILEGE_SET);
	}

	protected void processDAVDisplayName() {

		DocUtil.add(successPropElement, CalDAVProps.DAV_DISPLAYNAME,
	...
	protected void processDAVGetContentType() {
		DocUtil.add(successPropElement, CalDAVProps.DAV_GETCONTENTTYPE, resource.getContentType());
	}
	...
	protected void processDAVGetLastModified() {
		DocUtil.add(successPropElement, CalDAVProps.DAV_GETLASTMODIFIED, resource.getModifiedDate());
	}

	protected void processDAVIsReadOnly() {
		DocUtil.add(successPropElement, CalDAVProps.DAV_ISREADONLY, resource.isLocked());
	}
	...
	protected void processDAVPrincipalCollectionSet() {
		...
		if (resource != null) {
			Element calendarHomeSetElement = DocUtil.add(successPropElement, CalDAVProps.DAV_PRINCIPAL_COLLECTION_SET);

			DocUtil.add(calendarHomeSetElement, CalDAVProps.createQName("href"),
					CalDAVUtil.getCalendarResourceURL(resource));
		} else {
			DocUtil.add(failurePropElement, CalDAVProps.DAV_PRINCIPAL_COLLECTION_SET);
		}
	}

	protected void processDAVPrincipalURL() {
		Element principalUrlElement = DocUtil.add(successPropElement, CalDAVProps.DAV_PRINCIPAL_URL);
	...
	protected void processDAVSupportedReportSet() {
		Element supportedResportSetElement = DocUtil.add(successPropElement, CalDAVProps.DAV_SUPPORTED_REPORT_SET);

		for (String reportSet : CalDAVMethod.SUPPORTED_CALDAV_REPORT_SET) {
			Element supportedResportElement = DocUtil.add(supportedResportSetElement,
					CalDAVProps.createQName("supported-report"));

			DocUtil.add(supportedResportElement, CalDAVProps.createQName("report"), reportSet);
		}
	}
