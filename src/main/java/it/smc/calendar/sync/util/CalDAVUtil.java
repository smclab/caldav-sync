...
import it.smc.calendar.sync.InvalidRequestException;
...
public class CalDAVUtil {
	...
	public static final Namespace NS_APPLE_URI = SAXReaderUtil.createNamespace("http://apple.com/ns/ical/");

	public static final Namespace NS_CALDAV_URI = SAXReaderUtil.createNamespace("C", "urn:ietf:params:xml:ns:caldav");

	public static final Namespace NS_CALENDAR_SERVER_URI = SAXReaderUtil.createNamespace("CS",
	...
	public static String getCalendarBookingURL(CalendarBooking calendarBooking)
		...
		sb.append("/webdav/");
		sb.append(calendarBooking.getCalendarResource().getCalendarResourceId());
		...
		sb.append(com.liferay.calendar.exporter.CalendarDataFormat.ICAL.getValue());

		return sb.toString();
	}

	public static String getCalendarColor(Calendar calendar) {
		return StringPool.POUND.concat(String.format("%06X", (0xFFFFFF & calendar.getColor())));
	}
	...
	public static String getResourceETag(Resource resource) {
		...
		return buildETag(String.valueOf(resource.getPrimaryKey()), modifiedDate);
	}

	public static boolean isAndroid(WebDAVRequest webDAVRequest) {
		String userAgent = webDAVRequest.getHttpServletRequest().getHeader(HttpHeaders.USER_AGENT);
	...
	public static boolean isAndroidCalDAVSyncAdapter(WebDAVRequest webDAVRequest) {
	...
	public static boolean isCalendarRequest(WebDAVRequest webDAVRequest) {
		return !isPrincipalRequest(webDAVRequest) && (webDAVRequest.getPathArray().length > 2);
	}
	...
	public static boolean isMacOSX(WebDAVRequest webDAVRequest) {
		...
		if (Validator.isNull(userAgent)) {
			...
			return userAgent.contains("OS X") || userAgent.contains("Core") || userAgent.contains("OS_X");
		}
	}
	...
	public static boolean isRequestContentXML(HttpServletRequest request) {
		...
		if (method.equals("PROPFIND") || method.equals("PROPPATCH") || method.equals("REPORT")) {
	...
	public static boolean isThunderbird(WebDAVRequest webDAVRequest) {
		...
		if (Validator.isNull(userAgent)) {
			...
			return userAgent.contains("Thunderbird") || userAgent.contains("Lightning");
		}
	}
